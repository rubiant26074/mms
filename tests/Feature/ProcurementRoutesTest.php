...
class ProcurementRoutesTest extends TestCase
{
    ...
    public function test_legacy_rfq_url_redirects_to_native_route(): void
    {
        $admin = User::query()->where('username', 'admin')->firstOrFail();

        $this->actingAs($admin)
            ->get('/index.php?page=purch-rfq&status=draft')
            ->assertRedirect(route('procurement.rfqs.index', ['status' => 'draft']));
    }
