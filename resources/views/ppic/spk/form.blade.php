        ...
        <div class="row">
            ...
            <div class="col-md-8">
                ...
                <div class="card shadow-sm mb-3">
                    ...
                    <div class="card-body p-0"><table class="table table-bordered mb-0 small"><thead class="table-light text-center"><tr><th>Material</th><th width="80">Stok</th><th width="120">Dibutuhkan</th><th width="80">Unit</th></tr></thead><tbody>@forelse($materials as $m)<tr><td>{{ $m['item_name'] }}</td><td class="text-center {{ ($m['current_stock'] ?? 0) < $m['qty_required'] ? 'text-danger fw-bold' : 'text-success' }}">{{ ($m['current_stock'] ?? 0) + 0 }}</td><td class="text-end fw-bold">{{ ceil($m['qty_required']) }}</td><td class="text-center">{{ $m['unit'] }}</td></tr>@empty<tr><td colspan="4" class="text-center py-4 text-muted">Pilih Sales Order untuk memuat kebutuhan material</td></tr>@endforelse</tbody></table></div>
                </div>
                <div class="card shadow-sm mb-3">
