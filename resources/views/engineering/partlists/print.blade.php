...
<div class="doc">
    ...
    <table class="table table-bordered table-sm">
        <thead class="table-light"><tr><th>No</th><th>Item No</th><th>Drawing No</th><th>Part Name</th><th>Qty</th><th>Material</th><th>Thick</th><th>L</th><th>W</th><th>Process</th><th>Notes</th></tr></thead>
        <tbody>
        @forelse($spk->partlists as $part)
            <tr><td>{{ $loop->iteration }}</td><td>{{ $part->item_no }}</td><td>{{ $part->drawing_no }}</td><td>{{ $part->part_name }}</td><td>{{ $part->qty + 0 }}</td><td>{{ $part->material }}</td><td>{{ $part->thickness }}</td><td>{{ $part->length + 0 }}</td><td>{{ $part->width + 0 }}</td><td>{{ $part->process }}</td><td>{{ $part->notes }}</td></tr>
        @empty
