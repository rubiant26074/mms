...
<head>
    ...
    <style>
        @page{size:A4 portrait;margin:0}body{font-family:Arial,sans-serif;font-size:11px;margin:0;padding:20px;color:#000}.box{max-width:800px;margin:auto;min-height:96vh;display:flex;flex-direction:column;position:relative}.watermark{position:absolute;top:45%;left:50%;transform:translate(-50%,-50%) rotate(-30deg);font-size:90px;font-weight:bold;color:rgba(200,0,0,.14);border:4px solid rgba(200,0,0,.14);padding:8px 30px}.header{border-bottom:2px solid #333;padding-bottom:10px;margin-bottom:15px;display:flex;justify-content:space-between}.doc-title-box{border:1.5px solid #000;padding:5px;display:inline-block;text-align:center;min-width:250px}.doc-title{font-size:18px;font-weight:bold;letter-spacing:1px}.info-table,.data-table,.summary-table,.footer-sig{border-collapse:collapse}.info-table{width:100%;margin-bottom:15px}.data-table{width:100%;margin-bottom:12px}.data-table th,.data-table td,.summary-table td,.footer-sig th,.footer-sig td{border:1px solid #000;padding:8px}.data-table th,.footer-sig th{background:#f2f2f2;text-transform:uppercase;font-size:10px}.text-center{text-align:center}.text-right{text-align:right}.summary-table{width:42%;margin-left:auto;margin-bottom:15px}.summary-table .lbl{background:#f9f9f9;font-weight:bold}.summary-table .grand td{font-weight:bold;border-top:2px solid #000}.notes-box{border:1px solid #000;background:#fafafa;padding:8px;margin-bottom:20px}.footer-sig{width:100%;table-layout:fixed;margin-top:10px}.footer-sig td{height:92px;text-align:center;vertical-align:bottom}.sig-name{font-weight:bold;text-decoration:underline;display:block}.page-footer{margin-top:auto;text-align:center;border-top:1px solid #ccc;padding-top:10px}.footer-comp-name{font-size:14px;font-weight:bold;display:block}@media print{.no-print{display:none}}
    </style>
</head>
<body onload="window.print()">
@php
    $totalBruto = $order->items->sum(fn($row) => (float) $row->subtotal);
    $dpp = max($totalBruto - (float) $order->discount_amount, 0);
    $ppn = $dpp * (((float) $order->ppn_percent) / 100);
    $approved = in_array($order->status, ['approved_finance','sent','completed'], true);
    $pmApproved = in_array($order->status, ['approved_pm','approved_finance','sent','completed'], true);
    $finApproved = in_array($order->status, ['approved_finance','sent','completed'], true);
@endphp
<div class="box">
    @if(! $approved)<div class="watermark">DRAFT</div>@endif
    <div class="header">
        <div>@if($company->logo_path)<img src="{{ asset($company->logo_path) }}" style="max-height:60px;object-fit:contain" alt="Logo">@endif</div>
